    ...
    extends ClientCommand
{
        ...
        throws Exception
    {
        ...
        if (!error.isEmpty()) {
            throw usage(error, ctx);
        }

        if (keepRevision && latestRevision || latestRevision && revision != null || keepRevision && revision != null) {
            throw usage("Setting --keep-revision, --latest-revision, or --revision together is invalid.", ctx);
        }

        if (all && resume || resume && from != null || all && from != null) {
            throw usage("Setting --all, --resume, or --from together is invalid.", ctx);
        }
